...
2. Install [kind](https://kind.sigs.k8s.io/) (used to run PostgreSQL, Gitea, and the local data lake stack — Trino + Iceberg REST catalog + MinIO — in a Kubernetes cluster). kind requires a container runtime — Docker Desktop is the easiest option on macOS:
...
> Tip: `pnpm dev:up` brings up the cluster *and* every optional service (postgres, gitea, trino + minio + iceberg-rest) in one command. Use it once you've finished the rest of this setup and want a one-shot way to start your dev env. `pnpm dev:down` tears it all down. For the data lake stack specifically — Trino, MinIO, Iceberg REST — see [`docs/local-data-lake.md`](docs/local-data-lake.md).
