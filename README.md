...
- Local data lake stack — Trino + Iceberg REST catalog + MinIO, all in the same kind cluster ([details](docs/local-data-lake.md))
...
2. Install [kind](https://kind.sigs.k8s.io/) (used to run PostgreSQL, Gitea, the local data lake stack — Trino + Iceberg REST catalog + MinIO — and Airflow, all in a single Kubernetes cluster). kind requires a container runtime — Docker Desktop is the easiest option on macOS:
...
> Tip: `pnpm dev:up` brings up the cluster *and* every optional service (postgres, gitea, trino + minio + iceberg-rest, airflow) in one command. Use it once you've finished the rest of this setup and want a one-shot way to start your dev env. `pnpm dev:down` tears it all down. For the data lake stack specifically — Trino, MinIO, Iceberg REST — see [`docs/local-data-lake.md`](docs/local-data-lake.md). For Airflow (UI at <http://localhost:8088>, no credentials), see [`docs/local-airflow.md`](docs/local-airflow.md).
