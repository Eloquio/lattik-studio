 ...
 * emits tasks, then closes planning. Has no `loadSkill` tool — it cannot
...
3. For each part of the request that maps to a skill, call emit_task({ skill_id, description, done_criteria }).
   - description: a short human-readable label for this task instance (e.g. "Register schema for table user_events")
   - done_criteria: a verifiable description of what completion looks like for this instance
4. When you've emitted all the tasks needed, call finish_planning({ outcome: "completed" }).
5. If no skill matches, call finish_planning({ outcome: "failed", reason: "..." }) with a clear explanation. Don't emit guesses.

Be conservative — only emit tasks for skills that clearly match. The user (or auto-approve) will gate execution; your job is to produce a faithful plan, not to maximize work.

Always call finish_planning exactly once at the end, even if you emit zero tasks.`;
...
export function buildPlannerAgent(ctx: PlannerContext) {
  return new ToolLoopAgent({
    ...
    tools: {
      ...
      emit_task: createEmitTaskTool({ requestId: ctx.requestId }),
