...
export async function generateDags(): Promise<string[]> {
  ...
  // Filter by both kind + status in SQL — the (kind, status) composite index
  // (`idx_definitions_kind_status`) makes this O(matching rows) instead of
  // pulling every merged definition (entities, dimensions, metrics, …) just
  // to filter in JS. Same correctness, much smaller result set on workspaces
  // with lots of non-table definitions.
  const lattikTables = await db
    .select()
    .from(schema.definitions)
    .where(
      and(
        eq(schema.definitions.status, "merged"),
        eq(schema.definitions.kind, "lattik_table"),
      ),
    );

  const writtenKeys: string[] = [];
  // Collected payloads — written to S3 in parallel after the validation /
  // YAML-rendering loop completes, rather than one round-trip at a time.
  ...
  for (const table of lattikTables) {
    ...
    writes.push({ key, body: yamlContent });
    ...
    writes.push({ key: backfillKey, body: backfillYaml });
    writtenKeys.push(backfillKey);
  }

  // Fire all S3 puts in parallel — they're independent and each merged table
  // contributes 2 keys (forward + backfill). On a workspace with dozens of
  // tables the serial loop was tens of round-trips end-to-end.
  await Promise.all(writes.map((w) => putObject(S3_BUCKET, w.key, w.body)));
  ...
  const toDelete = existingKeys.filter(
    (k) => k.endsWith(".yaml") && !expectedKeys.has(k),
  );
  await Promise.all(toDelete.map((k) => deleteObject(S3_BUCKET, k)));
